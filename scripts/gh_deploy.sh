...
JAR_PATH=$REPOSITORY/$PROJECT_NAME/build/libs/*.jar
DEPLOY_PATH=$REPOSITORY/$PROJECT_NAME/
...
nohup java -jar -Duser.timezone=Asia/Seoul -Dspring.profiles.active=prod $DEPLOY_JAR --server.port=8080 >> $APPLICATION_LOG_PATH 2> $DEPLOY_ERR_LOG_PATH &
